 ...
 *
 * AR50 (sdm845 Naples) bus model. DDR math is the shared Venus4
 * worst-case model also used by AR50_LITE. Naples additionally has
 * a venus-llcc path with dedicated video slices (vidsc0/vidsc1),
 * so unlike LITE it must also vote LLCC; mirror the DDR total
 * (video reference/pixel traffic spills to LLCC, DT range caps it).
 */
...
static unsigned long __calculate_encoder(struct vidc_bus_vote_data *d)
{
	...
	/* Naples has venus-llcc with video slices; vote it like DDR */
	d->calc_bw_llcc = d->calc_bw_ddr;
